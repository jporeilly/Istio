...
change default namespace to STRICT mTLS:
```
kubectl apply -f 05_namespace-mTLS-STRICT.yaml
```
check policy:
```
kubectl describe pa -n default
```
change details service to ISTIO_MUTUAL with destination rule:  
```
kubectl apply -f 05_details-service-destination-rule.yaml
```
> back to the sleep container session
...
```
use the details API:  
details service is now ISTIO_MUTUAL:
```
curl http://reviews.default.svc.cluster.local:9080/reviews/1
```
however details service set as STRICT:  
